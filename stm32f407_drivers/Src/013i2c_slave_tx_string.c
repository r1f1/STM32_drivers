...
// some data
#define MAX_LEN				32
uint8_t message[MAX_LEN] = "Que te vote Txapote";
...
void I2C_ApplicationEventCallback(I2C_Handle_t *I2C1Handle, uint8_t AppEv){
	...
	if (AppEv == I2C_EV_DATA_REQ){
		...
		// Data is waiting for the slave to read so Slave has to read it
		commandcode = I2C_SlaveReceiveData(I2C1Handle->pI2Cx);

	}else if (AppEv == I2C_ERROR_AF){
		// This happens only during slave txing.
		// Master has sent the NACK so slave shoudl understand that master doesn't need more data
		commandcode = 0xff;
		cnt = 0;
